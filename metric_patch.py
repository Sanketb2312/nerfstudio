...
def split_image_whole(image_path, output_path1, output_path2, vmaf_scores, fvvdp_scores, psnr_scores):
    # Open the image
...
def split_image(image_path, output_path1, output_path2, vmaf_scores, fvvdp_scores, psnr_scores, patch_size=100):
    # Open the image
    img = Image.open(image_path)
    ...
    for i in range(num_patches_x):
        for j in range(num_patches_y):
            ...
            # Crop the patch from both original and rendered images
            patch1 = img.crop((left, upper, right, lower))
            patch2 = img.crop((left + width // 2, upper, right + width // 2, lower))

            # Build the output paths for the patch images
            patch_output_path1 = output_path1.replace(".jpg", f"_patch_{i}_{j}.jpg")
            patch_output_path2 = output_path2.replace(".jpg", f"_patch_{i}_{j}.jpg")
            
            print(f"Filename: {patch_output_path1}")

            # Save the patch images
            patch1.save(patch_output_path1)
            patch2.save(patch_output_path2)

            # Compute metrics for each patch
            call_ffmpeg(patch_output_path1, patch_output_path2, vmaf_scores)
            call_psnr(patch_output_path1, patch_output_path2, psnr_scores)
            #call_fvvdp(patch_output_path1, patch_output_path2, fvvdp_scores)
